...
import { hakiDisplayLines, hakiStarsLine } from "./haki.js";
import { roundNearestFive } from "./stats.js";
import { parseHaki } from "./haki.js";
...
export function buildCardEmbed(card, ownedEntry, viewer) {
  ...
  if (card.specialAttack) {
    const smin = Array.isArray(card.specialAttack.range) ? (card.specialAttack.range[0] || 0) : 0;
    const smax = Array.isArray(card.specialAttack.range) ? (card.specialAttack.range[1] || 0) : 0;
    statsParts.push(`**Special:** ${card.specialAttack.name} (${smin}-${smax} damage)`);
  }
  ...
  if (card.ability) {
    // Hide explicit numeric boost values in base card view (pulls/base views should not show exact boosts)
    // But show full for special ranks like Z
    const abilitySafe = (card.rank === 'Z') ? String(card.ability) : String(card.ability).replace(/\d+\s*%/g, '').trim();
    statsParts.push(`**Effect:** ${abilitySafe}`);
  }
  ...
  // Haki display for base/pull embeds — only show if card supports any haki
  if (Array.isArray(card.haki) && card.haki.length > 0) {
    const hakiLines = hakiDisplayLines(card);
    if (hakiLines && hakiLines.length) {
      descParts.push("");
      descParts.push("**Haki’s**");
      descParts.push(...hakiLines);
    }
  }
...
export function buildDropEmbed(card, level = 1, viewer) {
  ...
  if (card.ability) {
    const abilitySafe = String(card.ability).replace(/\d+\s*%/g, '').trim();
    statsParts.push(`**Effect:** ${abilitySafe}`);
  }
  ...
  // Haki display for drop/pull embeds — only show if card supports any haki
  if (Array.isArray(card.haki) && card.haki.length > 0) {
    const hakiLines = hakiDisplayLines(card);
    if (hakiLines && hakiLines.length) {
      descParts.push("");
  ...
  if (viewer && typeof viewer.displayAvatarURL === 'function') embed.setFooter({ text: footerText, iconURL: 'https://files.catbox.moe/e4w287.webp' });
  else embed.setFooter({ text: footerText });
...
export function buildUserCardEmbed(card, ownedEntry, viewer, equippedWeapon = null) {
  ...
  // Apply per-owner Armament Haki boost if present (display only)
  const parsed = parseHaki(card);
  const ownedHaki = (ownedEntry && ownedEntry.haki) ? ownedEntry.haki : {};
  let armamentMultiplier = 1;
  if (parsed.armament && parsed.armament.present) {
    const stars = Math.max(0, Number(ownedHaki.armament || 0));
    armamentMultiplier = 1 + 0.05 + (stars * 0.10);
    baseHealth = baseHealth * armamentMultiplier;
    baseAttackMin = baseAttackMin * armamentMultiplier;
    baseAttackMax = baseAttackMax * armamentMultiplier;
  }
  ...
  if (userHealth > 0) {
    ...
    if (hpBoost > 0) healthText += ` (includes +${hpBoost} from weapon)`;
    if (armamentMultiplier > 1) {
      const pct = Math.round((armamentMultiplier - 1) * 100);
      healthText += ` (includes +${pct}% from Armament Haki)`;
    }
  ...
  if (card.ability) {
    // In user-owned view we can show the effect text as-is, but boosts are shown separately below if present.
    statsParts.push(`**Effect:** ${card.ability}`);
  }

  // If user has a generated boost assigned to this owned entry, show it here
  if (ownedEntry && ownedEntry.boost) {
    const b = ownedEntry.boost;
    const bparts = [];
    if (b.atk) bparts.push(`ATK +${b.atk}%`);
    if (b.hp) bparts.push(`HP +${b.hp}%`);
    if (b.special) bparts.push(`SPECIAL +${b.special}%`);
    if (bparts.length) statsParts.push(`**Boosts:** ${bparts.join(' • ')}`);
  }
  ...
  if (card.signatureWeapon) {
    ...
    if (showSig) {
      const sigWeapon = getCardById(card.signatureWeapon);
      if (sigWeapon) {
        statsParts.push(`**Signature Item:** ${sigWeapon.name}`);
      }
    }
  }
  ...
  // Haki star display for user-owned view — only show for cards that support haki
  if (Array.isArray(card.haki) && card.haki.length > 0) {
    const hakiStars = hakiStarsLine(card, ownedEntry);
    if (hakiStars) {
      descParts.push("");
      descParts.push("**Haki’s**");
      descParts.push(`Armament: ${hakiStars.armament}`);
      descParts.push(`Observation: ${hakiStars.observation}`);
      descParts.push(`Conqueror's: ${hakiStars.conqueror}`);
    }
  }
